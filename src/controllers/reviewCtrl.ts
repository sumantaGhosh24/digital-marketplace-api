...
const reviewCtrl = {
  getReviews: async (req: Request, res: Response) => {
    try {
      const features = new APIFeatures(
        Review.find().populate("product", "_id title description thumbnail"),
        req.query
      )
        .paginating()
        .sorting()
  ...
  getProductReview: async (req: Request, res: Response) => {
    try {
      const review = await Review.findById(req.params.product)
        .populate("product", "_id title description thumbnail")
